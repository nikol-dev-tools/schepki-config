...
## 10. План разработки: Subscription Reminder (Следующая щепка)

**Концепция:** Трекер подписок и триалов для мелких SaaS-сервисов с локальным хранением данных.

### Архитектура и уроки (Boilerplate v2.0 + Стандарт v3)
- **Аналитика:** `first_open` (один раз), Custom Dimensions (`extension_id`, `extension_version`), продуктовые события (`sub_added`, `sub_deleted`).
- **Иконки:** Тёмный фон (`#0a0e1a`), неоновые акценты, строгий финансовый стиль. Исходники в `subscription-reminder-assets`.
- **UX-логика:** Донат-баннер с 4-го запуска, Rate Us на 10-й запуск, UTM-метки, Remote Config.
- **Языки (i18n):** EN, DE, FR, ES, PT (фокус на платёжеспособные рынки).

### Ключевые фичи (MVP)
1. **Добавление подписки:** Название, сумма, валюта, дата списания, частота, **ссылка на отмену**.
2. **Список и сортировка:** Ближайшие списания сверху.
3. **Шок-эффект:** Блок "Вы тратите: $X в месяц / $Y в год".
4. **Уведомления (Push):** За 3 дня и за 1 день до списания через `chrome.notifications`.
5. **Live Badge:** Красная цифра на иконке расширения, если есть списания в ближайшие 3 дня.
6. **Приватность:** Все данные хранятся только локально в `chrome.storage.local`.
